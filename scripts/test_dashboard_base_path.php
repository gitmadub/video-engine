...
function dashboard_base_path_assert(bool $condition, string $message): void
{
    if (!$condition) {
...
function dashboard_base_path_wait_for_server(string $baseUrl, int $attempts = 50): void
{
    for ($i = 0; $i < $attempts; $i++) {
        $curl = curl_init(rtrim($baseUrl, '/') . '/');
    ...
    throw new RuntimeException('Base-path browser smoke server did not start in time.');
}

function dashboard_base_path_find_listening_pid(int $port): ?int
{
...
function dashboard_base_path_insert_ready_video(PDO $pdo, int $userId, string $publicId, string $title, int $sizeBytes): int
{
    $now = ve_now();
    $stmt = $pdo->prepare(
        'INSERT INTO videos (
...
function dashboard_base_path_add_stats(int $videoId, int $userId, string $date, int $views, int $bandwidthBytes): void
{
    for ($i = 0; $i < $views; $i++) {
...
function dashboard_base_path_seed_user_earnings(PDO $pdo, int $userId, string $statDate, int $earnedMicroUsd): void
{
    $now = ve_now();
    $stmt = $pdo->prepare(
...
$node = 'node';
$dbPath = $root . DIRECTORY_SEPARATOR . 'storage' . DIRECTORY_SEPARATOR . 'dashboard-base-path.sqlite';
$port = 18083;
$baseUrl = 'http://127.0.0.1:' . $port;
$pathPrefix = '/video-engine';
$prefixedBaseUrl = $baseUrl . $pathPrefix;

@unlink($dbPath);

$env = [
    'VE_DB_DSN' => 'sqlite:' . str_replace('\\', '/', $dbPath),
    'VE_APP_KEY' => 'dashboard-base-path-app-key',
    'VE_BASE_PATH' => $pathPrefix,
];
...
$_SERVER['REQUEST_URI'] = $pathPrefix . '/';
...
$pdo = ve_db();
$user = ve_create_user('dashboard_alias', 'dashboard-alias@example.com', 'DashPass123');
$userId = (int) ($user['id'] ?? 0);
dashboard_base_path_assert($userId > 0, 'Base-path browser smoke user should be created.');

$alphaId = dashboard_base_path_insert_ready_video($pdo, $userId, 'alphaalias01', 'Alpha Alias Clip', 150 * 1024 * 1024);
$betaId = dashboard_base_path_insert_ready_video($pdo, $userId, 'betaalias002', 'Beta Alias Clip', 90 * 1024 * 1024);
...
dashboard_base_path_add_stats($alphaId, $userId, $sevenDaysAgoDate, 2, 300 * 1024 * 1024);
dashboard_base_path_add_stats($betaId, $userId, $sixDaysAgoDate, 1, 150 * 1024 * 1024);
dashboard_base_path_add_stats($alphaId, $userId, $yesterdayDate, 3, 400 * 1024 * 1024);
dashboard_base_path_add_stats($alphaId, $userId, $todayDate, 1, 25 * 1024 * 1024);
dashboard_base_path_add_stats($betaId, $userId, $todayDate, 1, 50 * 1024 * 1024);
dashboard_base_path_seed_user_earnings($pdo, $userId, $todayDate, 50000000);
...
$playbackInsert->execute([
    ':video_id' => $alphaId,
    ':session_token_hash' => hash_hmac('sha256', 'dashboard-base-path-session', ve_app_secret()),
    ':ip_hash' => hash_hmac('sha256', '127.0.0.1', ve_app_secret()),
    ':user_agent_hash' => hash_hmac('sha256', 'dashboard-base-path-agent', ve_app_secret()),
...
dashboard_base_path_assert(is_array($video), 'Expected base-path browser smoke video to exist.');

$_SERVER['HTTP_USER_AGENT'] = 'dashboard-base-path-agent';
...
try {
    ...
    dashboard_base_path_wait_for_server($prefixedBaseUrl);
    $serverPid = dashboard_base_path_find_listening_pid($port);
    echo "base-path browser server ready\n";

    $browserEnv = [
        'DASHBOARD_BROWSER_BASE_URL' => $baseUrl,
        'DASHBOARD_BROWSER_PATH_PREFIX' => $pathPrefix,
        'DASHBOARD_BROWSER_USER' => 'dashboard_alias',
    ...
    dashboard_base_path_assert($exitCode === 0, 'Playwright base-path dashboard browser smoke test failed.');
} finally {
